...
const strengthLabel = (s: number) => s <= 25 ? "Weak" : s <= 50 ? "Fair" : s <= 75 ? "Good" : "Strong";
const strengthColor = (s: number) => s <= 25 ? "bg-destructive" : s <= 50 ? "bg-yellow-500" : s <= 75 ? "bg-accent" : "bg-accent";
    ...
    <div className="flex min-h-screen">
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center gradient-primary relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative z-10 text-center px-12">
          ...
          <h2 className="font-heading text-3xl font-bold text-primary-foreground mt-8">Join AgentLab</h2>
          <p className="text-primary-foreground/80 mt-3 max-w-sm mx-auto">Start building intelligent AI agents in minutes.</p>
        </motion.div>
      </div>
      ...
      <div className="flex w-full lg:w-1/2 items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md space-y-6">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">Get started with AgentLab for free.</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={e => e.preventDefault()}>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input placeholder="John Doe" value={form.name} onChange={set("name")} className="pl-10" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type="email" placeholder="you@example.com" value={form.email} onChange={set("email")} className="pl-10" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type={showPassword ? "text" : "password"} placeholder="••••••••" value={form.password} onChange={set("password")} className="pl-10 pr-10" />
              ...
              {form.password && (
                <div className="space-y-1">
                  <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${strengthColor(strength)}`} style={{ width: `${strength}%` }} />
                  </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input type="password" placeholder="••••••••" value={form.confirm} onChange={set("confirm")} className="pl-10" />
              </div>
