    ...
    <div className="p-6 lg:p-8 space-y-8">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {agents.map((a, i) => (
            <motion.div key={a.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.05 }}
              className="glass-card rounded-xl p-5 shadow-card hover:shadow-glow transition-all group">
              <div className="flex items-center gap-3 mb-3">
                <div className="gradient-primary rounded-lg p-2"><Bot className="h-4 w-4 text-primary-foreground" /></div>
                <span className="text-xs font-medium text-primary bg-primary/10 rounded-full px-2 py-0.5">{a.category}</span>
              </div>
