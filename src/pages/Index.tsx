...
import { Bot, MessageSquare, Database, Shield, ArrowRight, Sparkles, Zap, Brain } from "lucide-react";
...
const features = [
  { icon: Bot, title: "AI Agent Builder", desc: "Create custom agents with templates, roles, and goals in minutes." },
  { icon: MessageSquare, title: "Real-time Chat Testing", desc: "Test your agents instantly with a live chat interface." },
  { icon: Database, title: "Knowledge Base Upload", desc: "Feed your agents with PDFs, docs, and custom data." },
  { icon: Shield, title: "Admin Control Panel", desc: "Full control over users, agents, and platform settings." },
];

const steps = [
  { num: "01", title: "Create Agent", desc: "Choose a template or start from scratch.", icon: Sparkles },
  { num: "02", title: "Customize Prompt", desc: "Define the role, tone, and behavior.", icon: Brain },
  { num: "03", title: "Test in Chat", desc: "Interact with your agent in real time.", icon: Zap },
];
      ...
      <div className="container relative mx-auto px-6">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <motion.div initial="hidden" animate="visible" className="space-y-6">
            <motion.div variants={fadeUp} custom={0} className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-sm text-primary">
              <Sparkles className="h-3.5 w-3.5" /> Now in Public Beta
            </motion.div>
            <motion.h1 variants={fadeUp} custom={1} className="font-heading text-4xl font-bold leading-tight md:text-6xl">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.3 }}>
            <div className="relative">
              ...
              <img src={heroImage} alt="AI Agent Platform" className="relative rounded-2xl shadow-card border border-border" />
            </div>
          </motion.div>
        </div>
      </div>
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {steps.map((s, i) => (
            <motion.div key={s.num} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i}
              ...
              <span className="font-heading text-xs font-bold tracking-widest text-primary uppercase">Step {s.num}</span>
      ...
      <div className="container mx-auto px-6">
        <div className="grid gap-8 md:grid-cols-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Build intelligent AI agents faster than ever.</p>
          </div>
          {[
