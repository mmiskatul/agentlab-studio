        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-slate-500 mt-2 font-medium text-lg">
              {agents.length} agent{agents.length !== 1 ? "s" : ""} total &mdash; {agents.filter(a => a.status === "Published").length} published
            </p>
          </motion.div>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Link href="/creator">
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-9 rounded-lg border-slate-200 text-slate-600 font-bold shadow-sm hover:bg-slate-50 hidden sm:flex"
                            >
                          ...
                          <Link href="/chat">
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-9 px-3 rounded-lg border-slate-200 text-blue-600 font-bold shadow-sm hover:bg-blue-50 hidden sm:flex"
                            >
                    ...
                    <tr>
                      <td colSpan={5} className="py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="font-bold text-slate-700">No agents match your search</p>
                          <button onClick={() => setSearch("")} className="text-sm font-bold text-blue-600 hover:underline">Clear search</button>
                        </div>
                      </td>
                    </tr>
                  )}
        ...
        {pendingDelete && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-[2rem] p-8 shadow-2xl max-w-md w-full border border-slate-200/60"
            >
              ...
              <p className="text-slate-500 font-medium text-sm leading-relaxed mb-8">
                You are about to permanently delete <span className="font-bold text-slate-800">&ldquo;{pendingDelete.name}&rdquo;</span>. This will remove all its configuration, knowledge base attachments, and chat history. This action cannot be undone.
              </p>
              <div className="flex gap-3">
                <Button
                  variant="outline"
                  onClick={() => setPendingDelete(null)}
                  className="flex-1 rounded-xl border-slate-200 font-bold shadow-sm h-11"
                >
                  Cancel
                </Button>
                <Button
                  onClick={confirmDelete}
                  className="flex-1 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold shadow-md h-11"
                >
