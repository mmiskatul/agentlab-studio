...
import { MessageSquare, Calendar, Search, ArrowRight, Trash2, Clock, Bot } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import Link from "next/link";

const chatHistory = [
  { id: 1, title: "Q3 Revenue Analysis", agent: "Data Analyst Pro", preview: "Revenue grew by 14% compared to Q2...", date: "Today, 10:42 AM", duration: "12m", msgs: 24 },
  ...
  { id: 3, title: "Docker Compose Debug", agent: "DevOps Helper", preview: "Your docker-compose.yml looks correct but...", date: "Oct 12, 2024", duration: "45m", msgs: 56 },
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-72">
            ...
            <Input placeholder="Search conversations..." className="pl-11 h-12 bg-white border-slate-200/60 rounded-xl shadow-sm font-medium" />
          </div>
          <Button variant="outline" className="h-12 rounded-xl border-slate-200/60 shadow-sm font-bold shrink-0 px-5"><Calendar className="w-4 h-4 mr-2" /> Date</Button>
        </div>
      </div>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 bg-slate-100 px-2.5 py-1 rounded-md shrink-0 flex items-center gap-1">
                    <Bot className="w-3 h-3" /> {chat.agent}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-4 text-xs font-semibold text-slate-400">
                  ...
                  <span>&bull;</span>
                  <span className="flex items-center gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> {chat.msgs} msgs</span>
                  <span>&bull;</span>
                  <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {chat.duration}</span>
                </div>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl text-slate-400 hover:text-red-600 hover:bg-red-50"><Trash2 className="w-5 h-5" /></Button>
                <Link href="/dashboard/chat">
                  <Button variant="outline" className="h-10 rounded-xl border-slate-200/60 shadow-sm font-bold">Resume <ArrowRight className="w-4 h-4 ml-2" /></Button>
                </Link>
              </div>
            </motion.div>
          ))}
